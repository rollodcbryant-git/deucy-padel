...
export function AuctionIntroModal({ open, hasPledged, onViewGallery, onAddPledge, onSkip }: AuctionIntroModalProps) {
  return (
      ...
      <DialogContent className="sm:max-w-sm [&>button]:hidden">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            {!hasPledged && (
              <Button onClick={onAddPledge} className="w-full touch-target bg-gradient-primary hover:opacity-90">
                <Gift className="h-4 w-4 mr-2" />
                Add my pledge
              </Button>
            )}
            <Button onClick={onViewGallery} variant={hasPledged ? 'default' : 'outline'} className={`w-full touch-target ${hasPledged ? 'bg-gradient-primary hover:opacity-90' : ''}`}>
              View Auction House
            </Button>
